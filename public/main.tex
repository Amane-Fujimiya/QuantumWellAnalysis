...
{\mathcal{D}_{-}^{(-)}}
\end{equation}
and the photon-dressing computation on multi-photon structure, field dependency on $\mathbf{E}_{0}$, and tie oscillation term $\mathcal{W}_{l,S}$, accordingly, as
\begin{equation}
\mathcal{W}_{l,S}(\mathbf{q}_{\perp})
=
...
Functionally, the interest in this part stems from the fact that, this term describes the contribution of the electron-phonon interaction Hamiltonian to the time evolution of the correlation function $F$, of which we derived earlier. Let us then consider the general expression
\begin{equation}
\begin{split}
...
and
$b_{m,\vec{q}}^{\dagger} b_{m,\vec{q}}$. This basically assumes BBGKY-like hierarchy, of which we keep only number-operator type terms, the structural tradeoff is to assume that no long-lieve coherence between different $k$-states can be observed othe current operator, there is no phonon coherence, which requires channel control, and quasi-classical occupation numbers, which might break under strong correlation.
Accordingly, we choose the indices for consistency check, as per choice for conservation law in expression of operator algebra,
\begin{itemize}
...
\cos \Omega t,
\end{equation}
we obtain, using the Floquet phase factor dressing, and admit the momentum modulation term on Jacobi-Anger form, 
\begin{equation}
\exp\left[
\int_{t'}^{t}
\frac{-ie}{m^* c}
